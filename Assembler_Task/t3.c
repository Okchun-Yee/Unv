...
void PassI(char *buf)
{
	...
	{
		if(!stricmp(Sen._operator,"dw")) strcpy(Symbol[j].word_type,"w");
		else if(!stricmp(Sen._operator,"db")) strcpy(Symbol[j].word_type,"b");
...
void PassII(char *buf)
{
	...
	if(i)
	{
		...
		{
			while(stricmp(Re[g[j].reg_name,Sen.operand[0]))
				j++;
